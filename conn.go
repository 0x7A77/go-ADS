...
// Connection
func NewConnection(ip string, targetNetId, sourceNetid string, port, targetPort, sourcePort uint16) (conn *Connection, err error) { /*{{{*/
	defer logger.Flush()

	conn = &Connection{ip: ip, sourceNetid: sourceNetid, targetNetid: targetNetId, port: port, targetPort: targetPort, sourcePort: sourcePort}
...
func (conn *Connection) Connect() {
	...
	conn.target = stringToNetId(conn.targetNetid)
	conn.target.port = conn.targetPort
	conn.source = stringToNetId(conn.sourceNetid)
	conn.source.port = conn.sourcePort

	log.Println("conn.source", conn.source)
